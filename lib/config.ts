...
export class Config {
    ...
    // When true (the default), the verify path also checks each slice's ADVISORY header md5 (bytes 7..22 =
    // md5 of the header's descriptive fields) and flags a mismatch as EHDRSUM. Now on by default: every legacy
    // pre-2015-scheme header has been re-stamped to the current scheme (2026-07 full-fleet pass), so nothing
    // false-flags. Opt out with STRUBS_VERIFY_HEADER_MD5=false. The live READ path never checks it (advisory).
